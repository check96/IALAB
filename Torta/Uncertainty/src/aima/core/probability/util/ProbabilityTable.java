...
public class ProbabilityTable implements CategoricalDistribution, Factor {
	...
	@Override
	public Factor maxOut(Record record, RandomVariable var) {
		Set<RandomVariable> maxVars = new LinkedHashSet<>(this.randomVarInfo.keySet());
		maxVars.remove(var);
		final ProbabilityTable maxedOut = new ProbabilityTable(maxVars);
		final Object[] termValues = new Object[maxedOut.randomVarInfo.size()];
		Iterator di = (possibleWorld, probability) -> {
			...
			if(maxedOut.getValues().length > 1)
				indice = maxedOut.getIndex(termValues);

			if (probability > maxedOut.getValues()[indice]) {
				maxedOut.getValues()[indice] = probability;

				Object obj = possibleWorld.remove(var);
				record.add(possibleWorld, var, obj);
			}
		};
		iterateOverTable(di);
